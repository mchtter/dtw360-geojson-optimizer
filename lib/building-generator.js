document.getElementById("submit").addEventListener("click", () => {
  ...
  geojson.features.forEach((feature, index) => {
    parsedData.push({
      ...
      ID: feature.properties.SAMID,
      type: feature.properties.type,
      buildingID: feature.properties.SAMID,

      height: feature.properties.height,
      doorX: feature.properties.Kapi_x.toFixed(6),
      doorY: feature.properties.Kapi_y.toFixed(6),
...
document.getElementById("download").addEventListener("click", () => {
  ...
  var zip = new JSZip();
  zip.file(`${geojson.name || "building_generation_data"}.json`, data);
  zip.generateAsync({ type: "blob" }).then(function (content) {
    saveAs(content, `${geojson.name || "building_generation_data"}_parsed.zip`);
  });
});
